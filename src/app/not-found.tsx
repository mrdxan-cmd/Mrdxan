  ...
  return (
    <section className="surface-dark min-h-[70vh]">
      <div className="container-x py-20 text-center sm:py-28">
        <p className="font-display text-7xl font-extrabold text-gradient-brand sm:text-8xl">404</p>
        ...
        <ul className="mx-auto mt-12 flex max-w-2xl flex-wrap justify-center gap-2">
          {services.map((s) => (
            <li key={s.slug}>
              <Link href={`/leistungen/${s.slug}`} className="inline-flex items-center gap-1.5 rounded-full border border-white/15 px-4 py-2 text-sm font-semibold text-ink-100 hover:border-brand-pink hover:text-white">
                {s.label}
